...
import { Plus, Trash2, Edit, FileText, X, Download, ArrowRightLeft, Settings2, ShieldCheck, ExternalLink, Loader2, MoreHorizontal, Eye, Printer, FileDown } from "lucide-react";
...
import { loadQuotationDocumentData, type QuotationDocData } from "@/lib/quotation-document";
...
function Quotations() {
  ...
  const [previewData, setPreviewData] = useState<QuotationDocData | null>(null);
  ...
  const exportPdf = async (q: Quote, lang: "ar" | "en" = "ar") => {
    if (pdfBusyId) return;
    setPdfBusyId(q.id);
    const loadingToast = toast.loading(`جاري تجهيز عرض السعر ${q.quote_number}...`);
    try {
      const data = await loadQuotationDocumentData(q.id);
      if (!data.items || data.items.length === 0) throw new Error("لا توجد بنود لهذا العرض");
      const { filename, size } = await downloadQuotationPdf(data, brand, lang);
      toast.success(`تم تحميل ${filename} (${(size / 1024).toFixed(0)} KB)`, { id: loadingToast });
    } catch (e: any) {
  ...
  const openPreview = async (q: Quote, lang: "ar" | "en" = "ar") => {
    if (pdfBusyId) return;
    setPdfBusyId(q.id);
    try {
      const data = await loadQuotationDocumentData(q.id);
      ...
      toast.error(`تعذر تحميل بيانات عرض السعر: ${e?.message ?? "خطأ"}`);
    } finally {
      setPdfBusyId(null);
    }
  };
                  ...
                  return (
                    <TableRow key={q.id}>
                      <TableCell className="font-mono">
                        <Link to="/quotations/$id" params={{ id: q.id }} className="text-primary hover:underline">
                          {q.quote_number}
                        </Link>
                      </TableCell>
                      ...
                      <TableCell className="text-left">
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button size="icon" variant="ghost" disabled={pdfBusyId === q.id}>
                              {pdfBusyId === q.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <MoreHorizontal className="w-4 h-4" />}
                            </Button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end" className="w-56">
                            <DropdownMenuItem onClick={() => navigate({ to: "/quotations/$id", params: { id: q.id } })}>
                            ...
                            <DropdownMenuItem onClick={() => openPreview(q, "ar")}>
                              <Eye className="w-4 h-4" /> معاينة PDF عربي
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => exportPdf(q, "ar")}>
                              <Download className="w-4 h-4" /> تحميل PDF عربي
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => exportPdf(q, "en")}>
                              <FileDown className="w-4 h-4" /> Download English PDF
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => { navigate({ to: "/quotations/$id", params: { id: q.id } }); setTimeout(() => window.print(), 800); }}>
                              <Printer className="w-4 h-4" /> طباعة
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            {!["accepted","rejected","expired"].includes(q.status) && !q.converted_order_id && (
                              <DropdownMenuItem onClick={() => requestApproval(q)}>
                                <ShieldCheck className="w-4 h-4" /> طلب موافقة
                              </DropdownMenuItem>
                            )}
                            {q.converted_order_id ? (
                              <DropdownMenuItem onClick={() => navigate({ to: "/orders/$id", params: { id: q.converted_order_id! } })}>
                                <ExternalLink className="w-4 h-4" /> عرض الطلب المرتبط
                              </DropdownMenuItem>
                            ) : q.status === "accepted" && (
                              <DropdownMenuItem onClick={() => convertToOrder(q)}>
                                <ArrowRightLeft className="w-4 h-4" /> تحويل إلى طلبية
                              </DropdownMenuItem>
                            )}
                            {isAdmin && (
                              <>
